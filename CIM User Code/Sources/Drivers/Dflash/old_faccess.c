...
static
MInt
get_page( DFLASH_T *pf, MUInt device, ushort *pcheck )
{
	MInt status;
...
{
	ushort check_main, check_sec;
	MInt st_main, st_sec;
	uint main_errors, sec_errors, diffs;

	main_errors = sec_errors = diffs = 0;
	for( df.page = 0 ; df.page < NUM_FW_PAGES ; ++df.page )
	{
		if( ( st_main = get_page( &df, MAIN_DEVICE, &check_main ) ) < 0 )
			++main_errors;
		if( ( st_sec = get_page( &df, SECONDARY_DEVICE, &check_sec ) ) < 0 )
			++sec_errors;
