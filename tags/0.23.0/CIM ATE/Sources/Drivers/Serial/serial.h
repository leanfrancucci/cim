 ...
 * 		conected to channel. 
 * 		Returns number of bytes actually taken
 * 		to send (normally the same as qty)
 * 		Receives a pointer to first char 'p'
 * 		and 'qty' to send 
 * 		Blocks till there is sufficient space
 */
void put_nchar( MUInt channel, const unsigned char *p, ushort qty );
 ...
 * 		Blocks till there is place. 
 */
void put_char( MUInt channel, const unsigned char c );
 ...
 */
MInt get_char( MUInt channel, unsigned char *p );

/*
 * set_baudrate:
 * 		Sets baudrate
 */
void set_baudrate( MUInt channel, MUInt bd );
